 ...
 * Design decisions:
 *
 * 1. Centralised: keeping both cached wrappers in one file (the
 ...
 *    (the query is viewer-agnostic: it returns the same recent posts
 *    regardless of who is asking). A service-role client is used for
 *    query simplicity, not privileged access; every column read here is
 *    already covered by the public-read RLS policies on `posts` and
 *    `users_public`.
 *
 ...
 *    missed.  The `tags: ['posts']` annotation means any Route Handler
 *    that calls `revalidateTag('posts', { expire: 0 })` immediately
...
/**
 * Cached top-playbooks query: 3 most recent playbooks in the last 7 days.
 * Revalidates every 600 s or on `revalidateTag('posts', ...)`.
 */
...
/**
 * Cached top-dives query: 3 most recent deep dives in the last 7 days.
 * Revalidates every 600 s or on `revalidateTag('posts', ...)`.
 */
