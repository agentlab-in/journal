 ...
 * users.id column the caller passed in. Empty Map on error or missing data
 * so the caller can skip rows whose author row vanished (FK is RESTRICT, so
 * this should not happen — defensive only).
 *
 * Mirrors the helper in `app/page.tsx`. Kept inline (not extracted to a
 * shared module) per the Phase 9 brief — we'll only have 2 callers so far.
 */
...
/**
 * Attach approved tags (max 2, slug-asc) to a set of post ids.
 * Mirrors the helper in `app/page.tsx`.
 */
