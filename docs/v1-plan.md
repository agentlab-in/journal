...
2. **`POST /api/posts`.** Auth required. Body: `{ title, summary, body_md, type, tag_slugs[], cover_image_url? }`. Steps:
   ...
   - For each tag slug not in DB: insert with `is_approved = false, created_by = user_id`. Emit `admin_tag_pending` notification for each admin.
   - Resolve wikilinks (see step 5) and write `post_links` rows.
   - Insert into `posts`, `post_tags`, `post_versions(version_no=1)`.
   - Return `{ id, url }`.
3. **`PATCH /api/posts/[id]`.** Auth required; author-only. Body: same shape minus type (type immutable). Steps:
   - Diff `body_md` from current. If changed, append new `post_versions` row (auto-incremented `version_no`), GC to last 20.
   - Re-compile HTML, re-resolve wikilinks (delete then re-insert this post's `post_links` rows).
   - Update `posts.body_md`, `body_html`, `title` (slug stays), `summary`, `cover_image_url`, `edited_at = now()`.
   - Adjust `post_tags` (diff add/remove). New tags create pending rows like step 2.
4. **`DELETE /api/posts/[id]`.** Auth required; author-only. Soft-delete (`is_deleted = true, deleted_by = 'author'`). Do NOT delete `post_links`, `comments`, `likes`, `bookmarks` — backlinks display logic excludes deleted posts at read time.
5. **Wikilink resolver (B4 semantics).**
   - Parse all `[[X]]` and `[[X|alias]]` from `body_md`.
   - For each target text, search posts:
     1. Match: `lower(title) = lower(X)` AND `is_deleted = false`.
     2. Tie-break order: (a) author = current user, (b) highest `like_count`, (c) most recent `published_at`.
   - If match found: store `(source_post_id, target_post_id, anchor_text=X)` in `post_links`; rewrite link href to canonical post URL.
   - If no match: render as `<span class="broken-wikilink" title="No post found for X">X</span>`.
...
8. **Tests.**
    ...
    - E2E: write post A with `[[B's Title]]`, publish; write post B with title "B's Title"; re-publish A (or write a fresh A that links to B post-creation); confirm `post_links` row exists and A's rendered HTML contains a link to B's URL.
...
- Submitting a new tag creates a pending tag and an admin notification.
- Wikilinks resolve at save time; backlink rows present in `post_links`.
- Soft-deletion sets `is_deleted = true` and the row is preserved.
